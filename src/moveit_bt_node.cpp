...
{
  ros::init(argc, argv, "moveit_bt_node");
  ros::NodeHandle nh("/moveit_bt_node");
  ...
  static const std::string LOGNAME = "moveit_bt_node";

  /* Otherwise robot with zeros joint_states */
  ros::Duration(1.0).sleep();

  ROS_INFO_STREAM_NAMED(LOGNAME, "Starting MoveIt_BT Tutorials...");
  ...
  visual_tools.publishText(text_pose, "MoveIt_BT Demo", rvt::WHITE, rvt::XLARGE);
  ...
  target_pose1.pose.position.x = 1.0;
  target_pose1.pose.position.y = -1.15;
  ...
  if (plan_solution1)
  {
    ...
    visual_tools.publishAxisLabeled(robot_start_state->getGlobalLinkTransform("flange"), "start_pose");
  ...
  //start_pose.position.x = 1.55;
  //start_pose.position.y = 0.4;
  //start_pose.position.z = 1.6;
  ...
  if (plan_solution2)
  {
    ...
    visual_tools.publishText(text_pose, "Start Pose", rvt::WHITE, rvt::XLARGE);
    visual_tools.publishAxisLabeled(robot_state.getGlobalLinkTransform("flange"), "start_pose");
    visual_tools.publishText(text_pose, "Goal Pose", rvt::WHITE, rvt::XLARGE);
    visual_tools.publishAxisLabeled(target_pose1.pose, "target_pose");
  ...
  if (plan_solution4)
  {
    ...
    visual_tools.publishAxisLabeled(robot_state.getGlobalLinkTransform("flange"), "start_pose");
    visual_tools.publishText(text_pose, "Goal Pose", rvt::WHITE, rvt::XLARGE);
    visual_tools.publishAxisLabeled(robot_start_state->getGlobalLinkTransform("flange"), "target_pose");
